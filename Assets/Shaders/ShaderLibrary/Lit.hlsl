...
#include "CoreShaderLibrary/Common.hlsl"
#include "CoreShaderLibrary/Shadow/ShadowSamplingTent.hlsl"
#include "CoreShaderLibrary/ImageBasedLighting.hlsl"
#include "CoreShaderLibrary/EntityLighting.hlsl"
...
#include "CoreShaderLibrary/UnityInstancing.hlsl"
...
{
    return SampleSingleLightmap(
        TEXTURE2D_PARAM(unity_DynamicLightmap, samplerunity_DynamicLightmap),
        uv,
...
float3 SubtractiveLighting(LitSurface s, float3 bakedLighting)
{
    ...
    float3 subtractiveLighting = bakedLighting - shadowedLightingGuess;
    subtractiveLighting = max(subtractiveLighting, _SubtractiveShadowColor);
    subtractiveLighting =
        lerp(bakedLighting, subtractiveLighting, _CascadedShadowStrength);
    return min(bakedLighting, subtractiveLighting);
}
...
float4 BakingShadows(VertexOutput input, LitSurface surface)
{
#if defined(LIGHTMAP_ON)
    #if defined(_SHADOWMASK) || defined(_DISTANCE_SHADOWMASK)
...
VertexOutput LitPassVertex (VertexInput input)
{
    ...
    output.dynamicLightmapUV = 
        input.dynamicLightmapUV * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
#endif
    return output;
}
    ...
    ) : SV_TARGET
{
...
#endif
    float4 bakedShadows = BakingShadows(input, surface);
    float3 color = input.vertexLighting * surface.diffuse;
        ...
        float shadowAttenuation = MixRealtimeAndBakedShadowAttenuation(
            CascadedShadowAttenuation(surface.position),
            bakedShadows, 0, surface.position, true 
        );
    ...
    for (int i = 0; i < min(unity_LightIndicesOffsetAndCount.y, 4); i++) {
        ...
        float shadowAttenuation = MixRealtimeAndBakedShadowAttenuation(
            ShadowAttenuation(lightIndex, surface.position), 
            bakedShadows, lightIndex, surface.position
        );
        color += 
            GenericLight(lightIndex, surface, shadowAttenuation);
    }
