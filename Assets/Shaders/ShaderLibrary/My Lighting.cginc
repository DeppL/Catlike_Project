...
InterpolatorsVertex MyVertexProgram (VertexData v) 
{
		...
		i.uv.zw = TRANSFORM_TEX(v.uv, _DetailTex);
			...
			float dispalcement = tex2Dlod(_DisplacementMap, float4(i.uv.xy, 0, 0)).g;
			dispalcement = (dispalcement - 0.5) * _DisplacementStrength;
			// v.vertex. += dispalcement;
			v.normal = normalize(v.normal);
			v.vertex.xyz += v.normal * dispalcement;
		#endif	
	#endif
...
{
#if HANDE_SHADOWS_BLENDING_IN_GI || ADDITIONAL_MASKED_DIRECTIONAL_SHADOWS
...
struct FragmentOutput {
		...
		#if defined(SHADOW_SHADOWMASK) && (UNITY_ALLOWED_MRT_COUNT > 4)
