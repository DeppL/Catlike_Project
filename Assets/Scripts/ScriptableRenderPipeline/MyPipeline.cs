/*
 * @Author: DeppL
 * @Date: 2019-11-19 22:35:59
 * @LastEditors  : DeppL
 * @LastEditTime : 2020-01-17 14:36:08
 * @FilePath: /Catlike_Project/Assets/Scripts/ScriptableRenderPipeline/MyPipeline.cs
 */
...
public class MyPipeline : RenderPipeline
{
    ...
    static int visibleLightAttenuationId =
    ...
    static int lightIndecesOffsetAndCountId =
...
#if UNITY_EDITOR
    static Lightmapping.RequestLightsDelegate lightmapingLightsDelegate =
...
#if UNITY_EDITOR
        Lightmapping.SetDelegate(lightmapingLightsDelegate);
#endif
    ...
    void ConfigureDitherPattern (ScriptableRenderContext context)
    {
        if (ditherSTIndex < 0) {
            ditherSTIndex = 0;
            lastDitherTime = Time.unscaledDeltaTime;
    ...
    void RenderShadows(ScriptableRenderContext content)
    {
        int split;
        ...
        // );
        content.ExecuteCommandBuffer(shadowBuffer);
        shadowBuffer.Clear();
        int tileIndex = 0;
        ...
        for (int i = mainLightExists ? 1 : 0; i < cull.visibleLights.Count; i++) {
            ...
            if (shadowData[i].x <= 0f) { // x => shadowStrength
                continue;
            }
            ...
            content.DrawShadows(ref shadowSettings);
