...
public class MyPostProcessingStack : ScriptableObject
{
    ...
    enum Pass
    {
        Copy, Blur, DepthStripes, ToneMapping
    };
    ...
    static void InitializeStatic()
    {
        ...
        material = new Material(
            Shader.Find("Hidden/My Pipeline/PostEffectStack")
        ) {
    ...
    //     if (blurStrength > 0) {
    //         Blit(cb, cameraColorId, width, height);
    //     }
    //     else {
    ...
    public void RenderAfterTransparent(
        CommandBuffer cb, int cameraColorId, int cameraDepthId,
        int width, int height, int samplers, RenderTextureFormat format
    )
    {
        if (blurStrength > 0) {
            if (toneMapping || samplers > 1) {
                cb.GetTemporaryRT(
                ...
                Blit(cb, resolvedTexId, width, height);
    ...
    void Blit (
        CommandBuffer cb, int cameraColorId, int width, int height
    ) {
        cb.BeginSample("Blur");
        if (blurStrength == 1) {
